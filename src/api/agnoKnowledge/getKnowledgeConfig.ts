...
/**
 * Get Knowledge Configuration API
...
export const getKnowledgeConfigAPI = async (
    params: GetKnowledgeConfigParams = {},
    baseUrl?: string
): Promise<GetKnowledgeConfigResponse> => {
    try {
        const queryString = buildQueryString(params)
        const endpoint = `/knowledge/config${queryString}`

        const response = await makeRequest(endpoint, { method: 'GET' }, baseUrl)
        const data: GetKnowledgeConfigResponse = await response.json()

        return data
    } catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Failed to get knowledge configuration'
        toast.error(`Error retrieving knowledge configuration: ${errorMessage}`)
        throw error
    }
}