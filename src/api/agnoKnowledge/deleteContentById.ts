...
/**
 * Delete Content by ID API
 * Permanently remove a specific content item from the knowledge base. 
 * This action cannot be undone.
 */
export const deleteContentByIdAPI = async (
    params: DeleteContentByIdParams,
    baseUrl?: string
): Promise<DeleteContentByIdResponse> => {
    try {
        ...
        const data: DeleteContentByIdResponse = await response.json()

        toast.success('Content deleted successfully')
        return data
    } catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Failed to delete content'
        toast.error(`Error deleting content: ${errorMessage}`)
        throw error
    }
}