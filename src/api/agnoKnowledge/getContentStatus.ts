...
/**
 * Get Content Status API
 * Retrieve the current processing status of a content item. 
 * Useful for monitoring asynchronous content processing progress and identifying any processing errors.
 */
export const getContentStatusAPI = async (
    params: GetContentStatusParams,
    baseUrl?: string
): Promise<GetContentStatusResponse> => {
    try {
        const { content_id, db_id } = params

        // Build endpoint with content_id and status path
        let endpoint = `/knowledge/content/${encodeURIComponent(content_id)}/status`
        ...
        const data: GetContentStatusResponse = await response.json()

        return data
    } catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Failed to get content status'
        toast.error(`Error retrieving content status: ${errorMessage}`)
        throw error
    }
}