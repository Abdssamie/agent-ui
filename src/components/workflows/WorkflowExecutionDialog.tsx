...
const getInitialJsonInput = (type: 'simple' | 'json', workflow: WorkflowSummary | null, generateTemplateFromSchema: (schema: any) => any) => {
    if (type === 'simple') {
        if (workflow?.metadata?.json_input_example) {
            return JSON.stringify(workflow.metadata.json_input_example, null, 2);
        }
        return '{\n"message": ""\n}';
    } else {
...
export const WorkflowExecutionDialog = ({
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChangeAction}>
            <DialogContent
                className="max-w-3xl max-h-[80vh] flex flex-col bg-background/95 backdrop-blur-sm rounded-xl border !scrollbar-thick border-primary/15">
                ...
                <div className="flex-1 flex flex-col gap-4 overflow-hidden px-2">
                    ...
                    {executionLogs.length > 0 && (
                        <div className="flex-1 flex flex-col gap-2 overflow-hidden">
                            ...
                            <div className="flex-1 overflow-hidden">
                                <div
                                    className="h-full rounded-xl border border-primary/15 bg-accent/50 p-4 overflow-y-auto">
                                    {workflowExecution ? (
                    ...
                    {executionLogs.length === 0 && !isExecuting && (
                        <div
                            className="flex-1 flex items-center justify-center rounded-xl border border-primary/15 bg-accent/50 p-8">
                            <div className="text-center space-y-2">
